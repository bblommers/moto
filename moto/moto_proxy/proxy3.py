...
class ProxyRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            print(f"{with_color(33, req.command.upper())} {host}{path}")
            if verbose:
    ...
    def decode_request_body(self, headers, body):
        if body is None:
            return body
        if True or headers.get("Content-Type", "") in ["application/x-amz-json-1.1", "application/x-www-form-urlencoded; charset=utf-8"]:
...
def main(HandlerClass=ProxyRequestHandler, ServerClass=ThreadingHTTPServer, protocol="HTTP/1.1"):
    ...
    ##################################################################################""")
    print("Using the CLI:")
    print(with_color(color=37, text=f"\texport HTTPS_PROXY=http://{sa[0]}:{sa[1]}"))
